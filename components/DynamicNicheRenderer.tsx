...
function FloatingWhatsApp({ waHref, label }: { waHref: string; label: string }) {
  return (
    <a
      href={waHref}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      className="wa-float-btn hidden md:flex fixed bottom-8 left-6 z-50 items-center justify-center w-14 h-14 rounded-full bg-green-500 hover:bg-green-600 text-white shadow-xl hover:scale-110 transition-all duration-200"
    >
...
export default function DynamicNicheRenderer({ lead }: { lead: PageData }) {
  ...
  return (
    <div className={`min-h-screen font-sans ${theme.pageBg}`}>
      ...
      {/* Floating WA button (desktop) + full-width sticky bar (mobile) */}
      <FloatingWhatsApp waHref={waHref} label={lead.hero.ctaText} />
      <StickyBar lead={lead} theme={theme} waHref={waHref} />
    </div>
  );
}
